...
def patch_remove_gate_and_trigger_cleanup():
    ...
    try:
        ...
        MISS.append("%s: %s (subscription gate removal)" % (rel, exc))
        return
    marker = "KAMIGRAM_CHANNEL_GATE_R82_DISABLED"
    if marker in text:
        return

    # Older target trees contain the r70 gate block; the pinned Telegram tree
    # does not, because r70's channel_guard is intentionally a no-op. In both
    # cases install one harmless resume marker so the final validation is
    # deterministic and the archive-only (>500) policy remains event-driven.
    old = """        /* KAMIGRAM_CHANNEL_GUARD (r70): после входа в аккаунт проверяем
...
"""
    new = """        /* KAMIGRAM_CHANNEL_GUARD / KAMIGRAM_CHANNEL_GATE_R82_DISABLED:
...
"""
    if old in text:
        ...
        anchor = "        checkFreeDiscSpace(0);\n"
        if anchor not in text:
            MISS.append("%s: resume insertion anchor not found (subscription gate removal)" % rel)
            return
        text = text.replace(anchor, new + anchor, 1)
    write(rel, text)
    DONE.append("subscription gate: remove mandatory AsuMeo check and trigger auto-cleanup on resume")
