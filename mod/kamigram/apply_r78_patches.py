#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""KamiGram r78 media, proxy-send, subscription, and folder fixes.
...
adding another download switch:
  ...
  * folder tabs receive a real KamiGram surface instead of a black runtime
    background.
...
def patch_folder_tabs():
    ...
    rel = "ui/Components/FilterTabsView.java"
    once(
        rel,
        "KAMIGRAM_FOLDER_SURFACE_R78",
        """    public FilterTabsView(Context context, Theme.ResourcesProvider resourcesProvider) {
        super(context);
""",
        """    public FilterTabsView(Context context, Theme.ResourcesProvider resourcesProvider) {
        super(context);
        /* KAMIGRAM_FOLDER_SURFACE_R78: all folder tabs, including «All personal»,
           are drawn on a KamiGram card rather than an opaque black fallback. */
        setWillNotDraw(false);
        setBackgroundColor(org.telegram.messenger.kamigram.ThemeHook.YORU_CARD);
""",
        "folders: use KamiGram card surface for the tab strip",
    )
    once(
        rel,
        "KAMIGRAM_FOLDER_SELECTOR_R78",
        "        selectorDrawable.setColor(Theme.getColor(tabLineColorKey, resourcesProvider));\n",
        """        selectorDrawable.setColor(Theme.getColor(tabLineColorKey, resourcesProvider));
        selectorDrawable.setColor(org.telegram.messenger.kamigram.ThemeHook.YORU_PURPLE); /* KAMIGRAM_FOLDER_SELECTOR_R78 */
""",
        "folders: selected-folder pill uses KamiGram accent",
    )
    once(
        rel,
        "KAMIGRAM_FOLDER_COLORS_R78",
        "        listView.setSelectorDrawableColor(Theme.getColor(selector, resourcesProvider));\n",
        """        listView.setSelectorDrawableColor(Theme.getColor(selector, resourcesProvider));
        selectorDrawable.setColor(org.telegram.messenger.kamigram.ThemeHook.YORU_PURPLE); /* KAMIGRAM_FOLDER_COLORS_R78 */
""",
        "folders: native color refresh cannot restore a black selector",
    )
...
def validate_sources():
    checks = [
        ...
        ("messenger/kamigram/ThemeHook.java", "KAMIGRAM_FOLDER_PALETTE_R78", "runtime folder palette"),
    ]
