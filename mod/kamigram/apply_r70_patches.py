...
def burn_read_menu():
    ...
    replace(chat, 'KAMIGRAM_MENU_BURN',
            ...
            '                   для одноразовых/по таймеру и в чатах с таймером. */\n'
            '                try {\n'
            '                    if (selectedObject != null && selectedObject.messageOwner != null) {\n'
            '                        final boolean kamigramEphemeral = ' + GHOST + '.isEphemeralMedia(selectedObject.messageOwner);\n'
            '                        final boolean kamigramChatTimer = currentChat != null && currentChat.ttl_period > 0;\n'
            '                        if (kamigramEphemeral || (kamigramChatTimer && !selectedObject.isOut())) {\n'
            '                            items.add("Сгореть");\n'
